...
const DashboardDetailView = ({
  ...
  const openProfile = (identifier) =>
    navigate(`/profile?search=${encodeURIComponent(identifier)}&view=${encodeURIComponent(identifier)}&section=directory`);
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                      {!isAdmin && (selectedEntry?.owner === userEmail || (selectedEntry?.contributors?.includes(userEmail) ?? false)) && (
                        <Button
                          variant="outlined"
                          size="small"
                          onClick={() => navigate(`/dashboard/entries/${selectedEntryId}/edit`)}
